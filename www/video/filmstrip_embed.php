    ...
    <html>
        <head>
            ...
            ?>
                ...
                div.waterfall-container {width:930px; margin: 0 auto;}
                #main, #header, #nav, div.waterfall-container, .page-wide {
...
function ScreenShotTable()
{
    ...
    if( count($tests) )
    {
        ...
        if( count($tests) == 1 ) {
            ...
            $width = array_key_exists("width", $_REQUEST) ? $_REQUEST["width"] : "930";
            echo CreateWaterfallHtml('', $requests, $tests[0]['id'], $tests[0]['run'], $tests[0]['cached'], $stepResult->getRawResults(),
                                     "&max=$end_seconds&mime=1&state=1&cpu=1&bw=1&width=$width", $tests[0]['step']);
            echo '<br><br>';
        } else {
          $waterfalls = array();
          foreach ($tests as &$test) {
